...
use types::square::{Square, get_printable_square_from_index};
...
fn main() {
    ...
    for i in knight_b1_moves.iter() {
        println!("{}", get_printable_square_from_index(i));
    }

    // Tests the legal moves for the bishop on c1 and prints them
    let square_c1 = Square::new(2); 
